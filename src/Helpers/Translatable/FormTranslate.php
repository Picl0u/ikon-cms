<?php
namespace Piclou\Ikcms\Helpers\Translatable;
...
class FormTranslate{
    ...
    public function __construct($modelName, $data = null)
    {
        ...
        $this->data = $data;
        $this->key = config('ikcms.translateKey');
    ...
    public function render(): string
    {
        if(!empty($this->translatableInputs) && !is_null($this->data)){
            $langs = array_diff(config('ikcms.languages'),[config('app.locale')]);
            if(!empty($langs)) {
    ...
    public function formRequest(Request $request)
    {
        ...
        if($request->method() == 'POST') {
            if(!config('ikcms.demo')) {
                return response(__("ikcms::admin.demo_error"), 401)->header('Content-Type', 'text/plain');
            }
            $id = Input::get('id');
            $lang = Input::get('lang');
            ...
            $content->update();
            return response()->json(["message" => __("ikcms::admin.translate_success")]);
        }

        return response( __("ikcms::admin.error"), 500)->header('Content-Type', 'text/plain');
    }
    ...
    {
        return view('ikcms::components.admin.translate.buttons',compact('langs'));
    }
    ...
    private function modal()
    {
        ...
        return view('ikcms::components.admin.translate.modal', compact('form','action'));
    }
